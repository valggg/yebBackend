...
import com.alex.server.service.*;
import com.alex.server.pojo.*;
import com.alex.server.service.*;
    ...
    public RespBean importEmployee(MultipartFile file) {
        ...
        try {
            ...
            list.forEach(employee -> {
                ...
                // 设置职称id
                employee.setJobLevelId(joblevelList.get(joblevelList.indexOf(new Joblevel(employee
                        .getJoblevel().getName()))).getId());
                // 设置职位id
                employee.setPosId(positionList.get(positionList.indexOf(new Position(employee
                        .getPosition().getName()))).getId());
        ...
        return RespBean.error("导入失败，请稍后再试！");
    }
}
